...
| Story order across unscripted scenes | §4.1 | **Provisional** | **Partly** | Stated in 0.41: scripted scenes precede unscripted ones, then `scene_number`, then row id. **The fixture cannot exercise it** — all thirteen scenes carry a heading, so the fallback never fires. Implemented and untested against an artifact. |
...
| Location variant selection | §12.17 | Provisional | **Partly** | Stated in 0.41: three-axis agreement, ties toward `is_baseline`, baseline fallback at zero. **`mismatches` is untested** — the fixture's chosen variant has none, so its shape rests on the reference implementation alone. |
