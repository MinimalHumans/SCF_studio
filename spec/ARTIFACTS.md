# Published artifacts

GENERATED by `schema/artifact_manifest.py`. Do not hand-edit.

Schema version **2.10**, tag `schema-2.10`.

## Addressing

A tag pins the tree; the raw URL at that tag is the artifact's
address. Nothing is duplicated per version — git already stores
the history, and a second copy in the working tree would only
raise the question of which one is real.

```
https://raw.githubusercontent.com/MinimalHumans/SCF_studio/schema-2.10/<path>
```

So the registry for this version is:

```
https://raw.githubusercontent.com/MinimalHumans/SCF_studio/schema-2.10/scf-core/registry/registry.json
```

`main` is a moving target and MUST NOT be used to pin a version.

## Verification

```sh
sha256sum -c spec/SHA256SUMS
```

## Artifacts

| Artifact | SHA-256 | What it is |
|---|---|---|
| `scf-core/registry/registry.json` | `89de0e535e2c8e3f…` | The normative field set. Spec §2.1. |
| `spec/scf-schema.sql` | `a41dd309ba3fee02…` | The physical DDL, dumped from initDatabase(). Spec §1.3. |
| `spec/registry.schema.json` | `448b8495efdfd09e…` | JSON Schema for registry.json, so it can be consumed without running the reference implementation. |
| `spec/scf.magic` | `95f9a444f4d42de8…` | magic(5) stanza for file(1). Spec §1.2. |
| `fixtures/hollow_creek.scf` | `a4ab586e1471f9b4…` | The conformance fixture. Its load-bearing properties are enumerated in spec/conformance.md §5.1. |

Full digests are in [`SHA256SUMS`](SHA256SUMS).

## Releasing a schema version

Step 6 of the change procedure in `docs/conventions.md` §8:

```sh
python3 schema/artifact_manifest.py
git tag -s schema-2.10 -m 'schema 2.10'
git push origin schema-2.10
```

The tag is what makes the URLs above resolve. Until it exists,
this manifest describes an address that 404s.
